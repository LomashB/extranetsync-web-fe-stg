...
const TextArea: React.FC<TextAreaProps> = ({
  ...
  const baseTextAreaClasses = `px-3 py-2 border rounded-md focus:outline-none focus:ring-2 transition-colors ${
    error 
      ? `border-[#d82c0d] focus:ring-[#d82c0d] focus:border-[#d82c0d] bg-[#fff4f4]` 
      : `border-[#8c9196] focus:ring-[#2c6ecb] focus:border-[#2c6ecb]`
  }`;
  
  const widthClasses = fullWidth ? 'w-full' : '';
  
  const textareaClasses = `${baseTextAreaClasses} ${widthClasses} ${className}`;
